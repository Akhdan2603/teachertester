...
2. Tambah data lesson-nya di `js/data.<criteria>.js` yang sesuai (`data.junior.js`
   / `data.kids.js` / `data.teens.js`) — **BUKAN** di `js/data.js`, file itu sekarang
   cuma shell kosong (`COURSE_DATA = {}`) yang selalu dimuat; isinya per criteria
   dipecah & dimuat dinamis (lihat `js/lazy-loader.js`,
   `rencana-10-10-non-security.md` bagian 5.1). Formatnya: tambah 1 property baru
   ke object yang di-`Object.assign(COURSE_DATA, {...})`, array `{num, title,
   objectives, objectives_en}` per lesson.
3. Tambah template teks laporannya di `js/templates.<criteria>.js` yang sesuai —
   sama seperti poin 2, `js/templates.js` sekarang juga cuma shell kosong.
4. Tambah mapping tab-nya di `COURSE_TAB_MAP` (`js/course-tab-map.js`) —
...
5. Kalau tab exam template-nya sudah ada di spreadsheet (Junior/Kids/Teens):
...
6. Jalankan 🩺 Health Check di aplikasi — pastikan tidak ada mapping yang
